...
*   **優先經驗回放 (PER)**：根據 TD-Error 大小決定取樣機率。

#### HW3-4: Rainbow DQN (加分篇)
*   **N-step Returns**：不再只看一步獎勵，而是整合未來 $N$ 步的資訊，加速 Q 值收斂。
*   **Noisy Networks**：在神經網路權重中加入學習出的雜訊，讓 Agent 能夠自發地進行更有意義的探索，取代傳統的 $\epsilon$-greedy。
...
在 `hw3_3_pl_dqn_random.png` 與 `hw3_4_rainbow_random.png` 中可以觀察到：
*   **Rainbow DQN** 在隨機環境下的起始學習速度通常更快，這歸功於 **N-step** 的獎勵快速傳播。
*   **Noisy Nets** 讓學習曲線更加平滑，減少了手動調參 $\epsilon$ 的依賴。
